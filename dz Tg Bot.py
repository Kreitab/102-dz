...
def start(message):
    bot.send_message(message.chat.id, "Привет, человек! Выберите действие:", reply_markup=create_keyboard())
...
def ask_for_k1(message):
    msg = bot.send_message(message.chat.id, "Введите коэффициент k1 для уравнения y = k1*x^2 + k2*x + k3:")
